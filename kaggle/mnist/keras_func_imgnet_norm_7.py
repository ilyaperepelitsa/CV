...
JSON_PATH = os.path.join(os.path.dirname(os.path.abspath("__file__")), "tf_specs.json")
...
working_params = hyper_params["keras_func_imgnet_norm_7"]
...
output_1 = Dense(FLAT_SIZE, activation = "relu",
    ...
    kernel_regularizer=regularizers.l2(0.01))(drop_1_2)
...
output_2 = Dense(FLAT_SIZE, activation = "relu",
    kernel_initializer = "truncated_normal",
    bias_initializer = "zeros",
    kernel_regularizer=regularizers.l2(0.01))(drop_2_2)
...
output = Dense(units = no_classes, activation = "softmax",
    ...
    kernel_regularizer=regularizers.l2(0.01))(norm)
...
learning_rate_reduction = ReduceLROnPlateau(monitor='val_acc',
                                            ...
                                            factor=0.2,
                                            min_lr=0.0001)
callbacks_list = [checkpoint, tensorbard, learning_rate_reduction]
...
y_pred = model.predict(target_x.values.reshape(target_x.shape[0], 28, 28, 1))
# y_pred

Y_pred_classes = np.argmax(y_pred, axis = 1)
# Y_pred_classes

submission = pd.DataFrame({"ImageId": target_x.index.values, "Label": Y_pred_classes })
# submission
